...
app.get('/make-server-c7e1f966/subscription/status', async (c) => {
  try {
    ...
    // If currentPeriodEnd is missing (e.g. complete-checkout didn't save it yet),
    // still grant access as long as status is active and we have a subscription ID.
    const isSubscriptionActive = (subData.status === 'active' || subData.status === 'trialing') &&
      !!subData.stripeSubscriptionId &&
      (!subData.currentPeriodEnd || now < subData.currentPeriodEnd);
